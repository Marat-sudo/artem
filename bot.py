...
def products_call(call):
    if db.user_in_db(call.message.chat.id):
        sv.basket(db.select_user_id(call.message.chat.id), call.data[9:])
        bot.answer_callback_query(call.id, text=f"Товар {db.select_name_from_id(call.data[9:])} добавлен в корзину", show_alert=True)
